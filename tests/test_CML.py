...
class TestCMLParser(unittest.TestCase):
    ...
    def setUp(self):
        os.chdir(
            os.path.join(primary, "test_molecules", "CML")
        )
        self.molecule = {
            'atoms': {'a1': 'H', 'a2': 'H', 'a3': 'O'},
            'bonds': {'b1': ('a1', 'a3', {'order': 1, 'chirality': None}),
    ...
    def test_parse(self):
        ...
        self.assertEqual(parser.molecule, self.molecule)
...
class TestCMLBuilder(unittest.TestCase):
    ...
    def test_from_Compound(self):
        with open("CML_1.cml", 'r') as cml_file:
            ...
            with tempfile.NamedTemporaryFile(
                    mode='r+', suffix='.cml', dir=os.getcwd()
            ) as tfile:
